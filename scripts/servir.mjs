...
createServer(async (req, res) => {
  try {
    ...
    // startsWith(RAIZ) a secas deja pasar /../hermano-con-prefijo/x
    if (abs !== RAIZ && !abs.startsWith(RAIZ + sep)) { res.writeHead(403).end('403'); return; }
...
}).listen(PUERTO, '127.0.0.1', () => console.log(`UI en  http://localhost:${PUERTO}`));
